...
export default function SolicitudesClient({ company }: Props) {
  ...
  async function submitReview(id: string, action: "APPROVE" | "REJECT", assignedCodigo?: string) {
  ...
  async function startApproveNewUpload(cr: ChangeRequest) {
    // For NEW_UPLOAD, show a código assignment form; for others approve immediately
    if (cr.type !== "NEW_UPLOAD") { submitReview(cr.id, "APPROVE"); return; }
    // Fetch suggested next código
                ...
                {!isRejecting && !isApproving ? (
                  <div style={{ display: "flex", gap: 8 }}>
                    <button
                      ...
                      onClick={() => startApproveNewUpload(cr)}
                  ...
                  <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: 14 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <input
                        ...
                        placeholder="DOC-001"
                        style={{ padding: "8px 11px", border: "1px solid #d1d5db", borderRadius: 8, fontSize: 13, width: 140, outline: "none" }}
                      />
                      <button
                        className="btn"
                        disabled={isProcessing}
                        onClick={() => submitReview(cr.id, "APPROVE", approveCodigo[cr.id] || undefined)}
                        style={{ background: "#dcfce7", color: "#166534" }}
                      >
                    ...
                    <p style={{ margin: "6px 0 0", fontSize: 11, color: "#94a3b8" }}>
                      Deja vacío para aprobar sin código.
                    </p>
                  </div>
                ) : (
                  <div>
