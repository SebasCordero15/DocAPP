  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", background: "#f1f5f9", fontFamily: `'${company.fontFamily}', Inter, system-ui, sans-serif` }}>
      ...
      <div style={{ background: brand, color: "#fff", padding: "12px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <span style={{ fontSize: 12, opacity: 0.75 }}>{total} {t("events")}</span>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, padding: "18px 20px", marginBottom: 20, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <div style={{ flex: "1 1 160px" }}>
            <label style={labelStyle}>{t("filters.action")}</label>
            <input
              type="text" placeholder={t("filters.actionPlaceholder")}
        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden" }}>
          {loading ? (
            ...
            <p style={{ padding: "40px", textAlign: "center", color: "#94a3b8" }}>{t("empty")}</p>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #f1f5f9" }}>
                  {[t("cols.when"), t("cols.action"), t("cols.user"), t("cols.resource"), t("cols.detail")].map((h) => (
                  ...
                  <tr key={l.id} style={{ borderBottom: "1px solid #f8fafc" }}>
                    ...
                    <td style={{ padding: "11px 18px", fontSize: 13, color: "#374151" }}>
                      {l.user ? (
                        ...
                        <span style={{ color: "#d1d5db" }}>{tc("sistema")}</span>
                      )}
                    </td>
                    <td style={{ padding: "11px 18px", fontSize: 12, color: "#64748b" }}>
                      {l.resourceType ?? <span style={{ color: "#d1d5db" }}>{tc("empty")}</span>}
                    </td>
                    <td style={{ padding: "11px 18px", fontSize: 13, color: "#64748b", maxWidth: 300, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={l.detail ?? ""}>
                      {l.detail ?? <span style={{ color: "#d1d5db" }}>{tc("empty")}</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {pageCount > 1 && (
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 20 }}>
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} style={pageBtn(page === 1)}>{t("prevPage")}</button>
            <span style={{ padding: "6px 12px", fontSize: 13, color: "#64748b" }}>{t("page")} {page} / {pageCount}</span>
            <button onClick={() => setPage((p) => Math.min(pageCount, p + 1))} disabled={page === pageCount} style={pageBtn(page === pageCount)}>{t("nextPage")}</button>
          </div>
        )}
      </div>
    </div>
